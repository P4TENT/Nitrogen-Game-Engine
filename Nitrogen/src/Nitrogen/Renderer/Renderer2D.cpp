...
namespace Nitrogen {
	...
	struct Renderer2DStorage {
		...
		uint32_t TextureSlotIdx = 1; // idx 0 = WhiteTexture
	};
	...
	{
		for (uint32_t i = 0; i < s_Data.TextureSlotIdx; i++)
	...
	void Renderer2D::DrawQuad(const glm::vec3& position, const glm::vec2& size, const glm::vec4& color)
	{
		...
		s_Data.QuadVertexBufferPtr->TexureIdx = texIndex; // 0 is s_Data.WhiteTexture
	...
	void Renderer2D::DrawQuad(const glm::vec3& position, const glm::vec2& size, const Ref<Texture2D>& texture, float tilingFactor, const glm::vec4& tintColor)
	{
		...
		for (uint32_t i = 1; i < s_Data.TextureSlotIdx; i++)
		{
		...
		{
			textureIndex = (float)s_Data.TextureSlotIdx;
			s_Data.TextureSlots[s_Data.TextureSlotIdx] = texture;
			s_Data.TextureSlotIdx++;
		}
		s_Data.QuadVertexBufferPtr->Position = position;
		s_Data.QuadVertexBufferPtr->Color = color;
		s_Data.QuadVertexBufferPtr->TexCoord = { 0.0f, 0.0f };
		s_Data.QuadVertexBufferPtr->TexureIdx = textureIndex;
		s_Data.QuadVertexBufferPtr->TilingFactor = tilingFactor;
		s_Data.QuadVertexBufferPtr++;
